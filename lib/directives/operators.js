...
var RepeatedOperationDirective = /** @class */ (function () {
    ...
    RepeatedOperationDirective.prototype.processParams = function (params, context, resolver) {
        ...
        return {
            args: resolver.processParameter(params, 'args', function (value) { return resolver.getArray(value, context); }, state)
        };
    };
    RepeatedOperationDirective.prototype.execute = function (params, context, resolver) {
        ...
        if (spec.args.length > 0) {
            var state = { source: spec.args, index: 0 };
            var subcontext = resolver.createChildStateContext(context, state);
            var left = resolver.resolveValue(spec.args[0], subcontext);
            for (state.index = 1; state.index < spec.args.length; state.index++) {
                ...
                var right = resolver.resolveValue(spec.args[state.index], subcontext);
...
var TypedRepeatedOperationDirective = /** @class */ (function () {
    ...
    TypedRepeatedOperationDirective.prototype.execute = function (params, context, resolver) {
        ...
        if (spec.args.length > 0) {
            var state = { source: spec.args, index: 0 };
            var subcontext = resolver.createChildStateContext(context, state);
            var left = resolver.resolveTypedValue(spec.args[0], subcontext, this.convertor);
            for (state.index = 1; state.index < spec.args.length; state.index++) {
                if (((_a = this.checkExitSignal) === null || _a === void 0 ? void 0 : _a.call(this, left)) === true) {
                    break;
                }
                var right = resolver.resolveTypedValue(spec.args[state.index], subcontext, this.convertor);
                left = this.callback(left, right);
            }
            return left;
        }
