...
var GetNestedValueDirective = /** @class */ (function () {
    ...
    GetNestedValueDirective.prototype.processParams = function (params, context, resolver) {
        ...
        return {
            source: resolver.processParameter(params, 'source', function (value) { return resolver.resolveValue(value, context); }, state),
    ...
    GetNestedValueDirective.prototype.execute = function (params, context, resolver) {
        ...
        var value = this.getViaUnresolvedPath(spec.source, spec.path, context, resolver, spec.default);
     ...
     */
    GetNestedValueDirective.prototype.getNestedValue = function (source, path, defaultValue) {
        var target = source;
        for (var _i = 0, path_1 = path; _i < path_1.length; _i++) {
            var step = path_1[_i];
            if (target != null) {
                var parent_1 = target;
                target = this.resolveStep(parent_1, step);
            }
            else
                return defaultValue;
        }
     ...
     */
    GetNestedValueDirective.prototype.getResolvedPath = function (path, context, resolver) {
     ...
     * @returns {unknown} retrieved value, if any
     */
    GetNestedValueDirective.prototype.getViaUnresolvedPath = function (source, path, context, resolver, defaultValue) {
        var state = { source: path };
        var subcontext = resolver.createChildStateContext(context, state);
        ...
        for (state.index = 0; state.index < path.length; state.index++) {
            ...
            if (target != null) {
                ...
                if (validStep == null)
                    return;
                target = this.resolveStep(parent_2, validStep);
            }
            else
                break;
        }
...
var GetLocalVariableDirective = /** @class */ (function (_super) {
    ...
    GetLocalVariableDirective.prototype.processParams = function (params, context, resolver) {
        var state = resolver.getResolutionState(context);
        return {
