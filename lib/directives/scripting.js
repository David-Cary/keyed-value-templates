...
var MultiStepDirective = /** @class */ (function () {
    ...
    MultiStepDirective.prototype.processParams = function (params, context, resolver) {
        ...
        return {
            steps: resolver.processParameter(params, 'steps', function (value) { return resolver.getArray(value, context); }, state)
        };
    };
    MultiStepDirective.prototype.execute = function (params, context, resolver) {
        var spec = this.processParams(params, context, resolver);
        var result = this.runSteps(spec.steps, context, resolver);
        return result.value;
    };
    /**
    ...
    MultiStepDirective.prototype.runSteps = function (steps, context, resolver) {
        ...
        var state = resolver.setParentStateOf({ source: steps }, localContext);
        ...
        for (state.index = 0; state.index < steps.length; state.index++) {
            ...
            var value = resolver.resolveValue(step, localContext);
...
var IterationDirective = /** @class */ (function (_super) {
    ...
    IterationDirective.prototype.processParams = function (params, context, resolver) {
        ...
        return {
            steps: resolver.processParameter(params, 'steps', function (value) { return resolver.getArray(value, context); }, state),
            for: resolver.processParameter(params, 'for', function (value) { return resolver.executeDirectiveFor(value, context); }, state),
    ...
    IterationDirective.prototype.execute = function (params, context, resolver) {
        ...
        if (typeof spec.for === 'object' && spec.for != null) {
            ...
            if (Array.isArray(spec.for)) {
                var state = resolver.setParentStateOf({ source: spec.for }, localContext);
                resolver.setResolutionState(localContext, state);
                ...
                var state = resolver.setParentStateOf({ source: valueMap }, localContext);
                resolver.setResolutionState(localContext, state);
...
var RepetitionDirective = /** @class */ (function (_super) {
    ...
    RepetitionDirective.prototype.processParams = function (params, context, resolver) {
        ...
        return {
            steps: resolver.processParameter(params, 'steps', function (value) { return resolver.getArray(value, context); }, state),
            from: resolver.processParameter(params, 'from', function (value) { return _this.resolveNumber(value, context, resolver, 1); }, state),
            to: resolver.processParameter(params, 'to', function (value) { return _this.resolveNumber(value, context, resolver, 1); }, state),
            rate: resolver.processParameter(params, 'rate', function (value) { return _this.resolveNumber(value, context, resolver, 1); }, state),
            return: params.return
        };
    };
...
var SetLocalValueDirective = /** @class */ (function () {
    ...
    SetLocalValueDirective.prototype.processParams = function (params, context, resolver) {
        ...
        return {
            ...
            path: resolver.processParameter(params, 'path', function (value) { return resolver.getArray(value, context); }, state),
            value: params.value,
            populate: resolver.processParameter(params, 'populate', function (value) { return resolver.resolveTypedValue(value, context, Boolean); }, state)
        };
    };
    SetLocalValueDirective.prototype.execute = function (params, context, resolver) {
        var spec = this.processParams(params, context, resolver);
        this.setUnresolvedValue(spec.source, spec.path, spec.value, context, resolver, spec.populate);
    };
    /**
     ...
     */
    SetLocalValueDirective.prototype.setNestedValue = function (source, path, value, populate) {
        ...
        if (path.length > 0) {
            ...
            for (var i = 0; i < maxIndex; i++) {
                if (target != null) {
                    var step = path[i];
                    var parent_1 = target;
                    target = this._getter.resolveStep(parent_1, step);
                    if (target == null && populate) {
                        target = typeof path[i + 1] === 'number' ? [] : {};
                        this.setObjectProperty(parent_1, step, target);
                    }
                }
                else
                    break;
            ...
            if (target != null) {
                var step = path[maxIndex];
                this.setObjectProperty(target, step, value);
            }
        }
    };
    /**
     ...
     * @param {boolean} populate - signals whether subcontainers should be created
     */
    SetLocalValueDirective.prototype.setUnresolvedValue = function (source, path, value, context, resolver, populate) {
        if (populate === void 0) { populate = false; }
        if (path.length > 0) {
            var state = { source: path };
            var subcontext = resolver.createChildStateContext(context, state);
            var target = source !== null && source !== void 0 ? source : subcontext;
            var maxIndex = path.length - 1;
            if (state.parent != null)
                state.parent.property = 'path';
            for (state.index = 0; state.index < maxIndex; state.index++) {
                var step = path[state.index];
                if (target != null) {
                    var resolvedStep = resolver.resolveValue(step, subcontext);
                    var validStep = this._getter.getValidStepFrom(resolvedStep);
                    if (validStep != null) {
                        var parent_2 = target;
                        target = this._getter.resolveStep(parent_2, validStep);
                        if (target == null && populate) {
                            target = typeof path[state.index + 1] === 'number' ? [] : {};
                            this.setObjectProperty(parent_2, validStep, target);
                        }
                    }
                    else {
                        target = null;
            ...
            if (target != null) {
                ...
                if (validStep != null) {
                    if (state.parent != null)
                        state.parent.property = 'value';
                    var resolvedValue = resolver.resolveValue(value, context);
                    this.setObjectProperty(target, validStep, resolvedValue);
                }
            }
            if (state.parent != null)
                state.parent.property = undefined;
        }
    };
    /**
